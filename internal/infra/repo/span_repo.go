...
func NewSpanRepo(client *mongo.Client, expireSeconds int32) *SpanRepo {
	return &SpanRepo{
		coll:          client.Database(SpanDB).Collection(SpanColl),
		expireSeconds: expireSeconds,
	}
}

func (r *SpanRepo) EnsureIndexes(ctx context.Context) error {
...
func (r *SpanRepo) InsertMany(ctx context.Context, spans []span.SpanEntity) error {
	if len(spans) == 0 {
		return nil
	}
...
func (r *SpanRepo) GetByTraceID(ctx context.Context, traceID string) ([]span.SpanEntity, error) {
...
func (r *SpanRepo) ListTraces(ctx context.Context, filter span.TraceFilter, skip, limit int64) ([]span.TraceEntity, error) {
...
func (r *SpanRepo) CountTraces(ctx context.Context, filter span.TraceFilter) (int64, error) {
	pipeline := mongo.Pipeline{
		{{Key: "$match", Value: buildTraceMatch(filter)}},
		{{Key: "$group", Value: bson.D{
			{Key: "_id", Value: "$trace_id"},
...
func buildErrorMatch(filter span.TraceFilter) bson.M {
...
func buildTraceMatch(filter span.TraceFilter) bson.D {
	match := bson.D{}
	if filter.AppID != nil {
